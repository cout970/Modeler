...
object GLTF {
    ...
    data class File(
            ...
            val accessors: List<Accessor>        = emptyList(),  // An array of accessors. An accessor is a typed view into a bufferView.
            ...
            val buffers: List<Buffer>            = emptyList(),  // An array of buffers. A buffer points to binary geometry, animation, or skins.
            val bufferViews: List<BufferView>    = emptyList(),  // An array of bufferViews. A bufferView is a view into a buffer generally representing a subset of the buffer.
            ...
            val meshes: List<Mesh>               = emptyList(),  // An array of meshes. A mesh is a set of primitives to be rendered.
            val nodes: List<Node>                = emptyList(),  // An array of nodes.
    ...
    data class Camera(
            val orthographic: OrthographicCamera?   = null, // An orthographic camera containing properties to create an orthographic projection matrix.
            val perspective: PerspectiveCamera?     = null, // A perspective camera containing properties to create a perspective projection matrix.
            val type: CameraType                    = CameraType.orthographic, // Specifies if the camera uses a perspective or orthographic projection. Based on this, either the camera's perspective or orthographic property will be defined.
            val name: String?                       = null, // The user-defined name of this object. This is not necessarily unique, e.g., an accessor and a buffer could have the same name, or two accessors could even have the same name.
            val extensions: String?                 = null, // Dictionary object with extension-specific objects.
            val extras: Any?                        = null  // Application-specific data.
    )
    ...
    data class Accessor(
            ...
            val type: Type              = Type.SCALAR, // Specifies if the attribute is a scalar, vector, or matrix.
            ...
            val sparse: Sparse?         = null,        // Sparse storage of attributes that deviate from their initialization value.
            val name: String?           = null,        // The user-defined name of this object. This is not necessarily unique, e.g., an accessor and a buffer could have the same name, or two accessors could even have the same name.
            val extensions: String?     = null,        // Dictionary object with extension-specific objects.
            val extras: Any?            = null         // Application-specific data.
    )
    ...
    data class Sparse(
            ...
            val indices: List<Accessor> = emptyList(), // Index array of size count that points to those accessor attributes that deviate from their initialization value. Indices must strictly increase.
            val values: List<Accessor>  = emptyList(), // Array of size count times number of components, storing the displaced accessor attributes pointed by indices. Substituted values must have the same componentType and number of components as the base accessor.
            val extensions: String?     = null,        // Dictionary object with extension-specific objects.
            val extras: Any?            = null         // Application-specific data.
    )
    ...
    data class Mesh(
            val primitives: List<Primitive> = emptyList(), // An array of primitives, each defining geometry to be rendered with a material.
    ...
    data class Material(
            ...
            val extensions: String?                         = null,             // Dictionary object with extension-specific objects.
            val extras: Any?                                = null,             // Application-specific data.
            val pbrMetallicRoughness: PbrMetallicRoughness? = null,             // A set of parameter values that are used to define the metallic-roughness material model from Physically-Based Rendering (PBR) methodology. When not specified, all the default values of pbrMetallicRoughness apply.
            val normalTexture: NormalTextureInfo?           = null,             // A tangent space normal map. The texture contains RGB components in linear space. Each texel represents the XYZ components of a normal vector in tangent space. Red [0 to 255] maps to X [-1 to 1]. Green [0 to 255] maps to Y [-1 to 1]. Blue [128 to 255] maps to Z [1/255 to 1]. The normal vectors use OpenGL conventions where +X is right and +Y is up. +Z points toward the viewer. In GLSL, this vector would be unpacked like so: vec3 normalVector = tex2D(normalMap, texCoord) * 2 - 1. Client implementations should normalize the normal vectors before using them in lighting equations.
            val occlusionTexture: OcclusionTextureInfo?     = null,             // The occlusion map texture. The occlusion values are sampled from the R channel. Higher values indicate areas that should receive full indirect lighting and lower values indicate no indirect lighting. These values are linear. If other channels are present (GBA), they are ignored for occlusion calculations.
            val emissiveTexture: TextureInfo?               = null,             // The emissive map controls the color and intensity of the light being emitted by the material. This texture contains RGB components in sRGB color space. If a fourth component (A) is present, it is ignored.
            ...
            val alphaMode: AlphaMode                        = AlphaMode.OPAQUE, // The material's alpha rendering mode enumeration specifying the interpretation of the alpha value of the main factor and texture.
    ...
    data class PbrMetallicRoughness(
            ...
            val metallicRoughnessTexture: TextureInfo?  = null,         // The metallic-roughness texture. The metalness values are sampled from the B channel. The roughness values are sampled from the G channel. These values are linear. If other channels are present (R or A), they are ignored for metallic-roughness calculations.
            val extensions: String?                     = null,         // Dictionary object with extension-specific objects.
            val extras: Any?                            = null          // Application-specific data.
    )
